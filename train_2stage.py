...
# 应用低通滤波器 (高斯模糊)
def apply_lowpass_filter(image_tensor, kernel_size=9, sigma=1.0):
...
def training(dataset, opt, pipe, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # image_sparse = render_pkg_sparse["render"]
        ...
        # loss_sparse += 0.01 * l1_loss_mask(rendered_depth, render_pkg_sparse["depth"])
        ...
        # image_eval = apply_lowpass_filter(image_eval.unsqueeze(0))
        # loss_sparse += 0.1 * ((1.0 - opt.lambda_dssim) * l1_loss_mask(image_eval, image_sparse_eval) + opt.lambda_dssim * (1.0 - ssim(image_sparse_eval, image_eval)))
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss,
                            [10000], scene, render, (pipe, background))
            ...
            if  iteration < opt.densify_until_iter and iteration not in clean_iterations:
                ...
                #     vert_id_to_keep = np.random.choice(np.arange(int(mask.sum())), size=min(int(mask.sum()), 5000), replace=False)
                #     # vert_id_to_keep = np.random.choice(np.arange(int(mask.sum())), size=int(0.01 * int(mask.sum())), replace=False)
                #     vert_id_to_keep = torch.tensor(vert_id_to_keep).cuda().long()
    ...
    progress_bar = tqdm(range(first_iter, opt.iterations), desc="Training progress sparse")
    for iteration in range(first_iter, opt.iterations + 1):
        if network_gui.conn == None:
            network_gui.try_connect()
        while network_gui.conn != None:
