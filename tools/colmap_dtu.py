...
def pipeline(scene, base_path, n_views):
    ...
    train_img_list = [c for idx, c in enumerate(img_list) if idx in train_idx]
    if n_views > 0:
    ...
    for img_name in train_img_list:
        os.system('cp ../images_2/' + img_name + '  images_2/' + img_name)
...
for n in [3]:
    for scene in ['scan8', 'scan21', 'scan30', 'scan31', 'scan34', 'scan38', 'scan40', 'scan41', 'scan45', 'scan55', 'scan63', 'scan82', 'scan103', 'scan110', 'scan114']:
        pipeline(scene, base_path = '/mnt/lab/zyl/models/FlowGS-final/dataset/dtu/', n_views = n)  # please use absolute path!
