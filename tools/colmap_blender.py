...
def pipeline(scene, base_path, n_views):
    ...
    # os.mkdir(view_path)
    os.chdir(view_path)
    os.system('rm -r created')
    os.system('rm -r triangulated')
    os.mkdir('created')
...
# for scene in ['chair', 'drums', 'ficus', 'hotdog', 'lego', 'materials', 'mic', 'ship']:# ['bonsai', 'counter', 'garden', 'kitchen', 'room', 'stump']:
for scene in ['chair', 'drums', 'hotdog', 'lego', 'materials', 'mic', 'ship']:# ['bonsai', 'counter', 'garden', 'kitchen', 'room', 'stump']:
    pipeline(scene, base_path = '/mnt/lab/zyl/models/NexusGS-anneal/dataset/nerf_synthetic/', n_views = 8)  # please use absolute path!
