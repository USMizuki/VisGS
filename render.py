...
from gaussian_renderer import render, render_point_cloud, render_filter_depth
import torchvision
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, args):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        rendering = render_filter_depth(view, gaussians, pipeline, background)
        point_cloud = render_point_cloud(view, gaussians, pipeline, background, scale_rate=0.00000001)["render"]
        gt = view.original_image[0:3, :, :]
        torchvision.utils.save_image(rendering["render"], os.path.join(render_path, view.image_name + '.png'))
        torchvision.utils.save_image(point_cloud, os.path.join(pc_path, view.image_name + '.png'))
...
def render_video(source_path, model_path, iteration, views, gaussians, pipeline, background, fps=30):
    ...
    for idx, view in enumerate(tqdm(views*10, desc="Rendering progress")):
        # view.world_view_transform = torch.tensor(getWorld2View2(pose[:3, :3].T, pose[:3, 3], view.trans, view.scale)).transpose(0, 1).cuda()
        # view.full_proj_transform = (view.world_view_transform.unsqueeze(0).bmm(view.projection_matrix.unsqueeze(0))).squeeze(0)
        # view.camera_center = view.world_view_transform.inverse()[3, :3]
        rendering = render(view, gaussians, pipeline, background)
