...
def training(dataset, opt, pipe, args):
    ...
    # gaussians_sparse.create_from_pcd(scene.point_cloud, gaussians.get_xyz, gaussians.get_features, gaussians.get_scaling, scene.cameras_extent, 0.01)
    with torch.no_grad():
    ...
    from utils.anneal_utils import cacl, transfer_gaussians, drop_gaussians
    score = cacl(gaussians_sparse, render, scene.getEvalCameras().copy(), (pipe, background), "psnr")
    t = 10
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        rendered_depth_sparse = render_pkg_sparse["depth"]
        ...
        if rendered_depth.shape[0] != 0 and iteration > 0 and opt.depth_weight != 0:
            if args.dataset_type == 'dtu':
                ...
                rendered_depth_sparse[bg_mask] = rendered_depth_sparse[~bg_mask].mean().detach()
            elif args.dataset_type == 'blender':
                flow_depth[bg_mask] = 0

            flow_depth = flow_depth.view(-1,1)
            rendered_depth = rendered_depth.view(-1,1)
            rendered_depth_sparse = rendered_depth_sparse.view(-1, 1)
            #depth_loss = 1 - pearson_corrcoef( flow_depth, rendered_depth)
            ...
            depth_loss_sparse = l1_loss_mask(flow_depth, rendered_depth_sparse)
            loss_sparse += opt.depth_weight * depth_loss_sparse
        ...
        with torch.no_grad():
            ...
            if  iteration < opt.densify_until_iter and iteration not in clean_iterations:
                ...
                # if iteration > opt.densify_from_iter and iteration % opt.downsample_interval == 0 and iteration <= opt.downsample_until_iter:
                ...
                #     grads = gaussians.xyz_gradient_accum / gaussians.denom
                #     grads[grads.isnan()] = 0.0
                #     mask = torch.where(torch.norm(grads, dim=-1) < scene.cameras_extent * 0.01, True, False)
                #     # vert_id_to_keep = np.random.choice(np.arange(int(mask.sum())), size=min(int(mask.sum()), 1000), replace=False)
                #     vert_id_to_keep = np.random.choice(np.arange(int(mask.sum())), size=int(opt.downsample_rate * int(mask.sum())), replace=False)
            ...
            if iteration % 1000 == 0 and int(gaussians.get_xyz.shape[0] * np.random.rand() * t * 0.001) > 0:
                for cam in scene.getPseudoCameras():
                    image = render(cam, gaussians, pipe, background)["render"]
                    cam.set_image(image)
                score = cacl(gaussians_sparse, render, scene.getPseudoCameras().copy(), (pipe, background), "psnr")
                while t > 10 * (0.1 ** int(iteration / 1000)):
                    pick_rate = np.random.rand() * t * 0.002

                    if np.random.rand() > 0.5:

                        transfer_mask = transfer_gaussians(gaussians, gaussians_sparse, pick_rate)

                        new_score = cacl(gaussians_sparse, render, scene.getPseudoCameras().copy(), (pipe, background), "psnr")
                        delta = new_score - score
                        if delta > 0:
                            score = new_score
                        else:
                            if math.exp(-delta / t) < np.random.rand():
                                gaussians_sparse.prune_points(transfer_mask, 0)
                            else:
                                score = new_score

                    else:

                        pruned_gaussians = drop_gaussians(gaussians_sparse, pick_rate * 10)
                        ...
                        if delta > 0:
                            ...
                            if math.exp(-delta / t) < np.random.rand():
                                gaussians_sparse.densification_postfix(*pruned_gaussians)
                            else:
                                score = new_score

                    t *= 0.97

                    print("Simulated annealing:", score, gaussians_sparse.get_xyz.shape[0])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--downsample_until_iter', type=int, default=5000)
    parser.add_argument('--downsample_interval', type=int, default=500)
    parser.add_argument('--downsample_rate', type=float, default=0.01)
